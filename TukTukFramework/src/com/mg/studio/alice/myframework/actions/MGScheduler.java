...
import com.mg.studio.alice.myframework.config.MGConfig;
import com.mg.studio.alice.myframework.util.collections.ConcurrentArrayHashMap;
...
public class MGScheduler {
    ...
    */
    ...
    private static MGScheduler _sharedScheduler = null;

    /** returns a shared instance of the Scheduler */
    public static MGScheduler sharedScheduler() {
        if (_sharedScheduler != null) {
            return _sharedScheduler;
        }
        synchronized (MGScheduler.class) {
            if (_sharedScheduler == null) {
                _sharedScheduler = new MGScheduler();
            }
            return _sharedScheduler;
        }
    }
    ...
    private MGScheduler() {
        ...
        try {
			impMethod = MGTimer.class.getMethod(updateSelector, Float.TYPE);
		} catch (Exception e) {
			impMethod = null;
		}
    ...
    public void tick(float dt) {
        ...
        for(ConcurrentArrayHashMap<Object, tHashSelectorEntry>.Entry e = hashForSelectors.firstValue();
            ...
            if( ! currentTarget.paused ) {                
	            ...
	            // so it is safe to ask this here 
	            // elt=elt->hh.next;
    ...
    */
    public void schedule(String selector, Object target, float interval, boolean paused) {
        ...
        if( element == null ) {
            ...
            assert element.paused == paused : "MGScheduler. Trying to schedule a selector with a pause value different than the target";
        }

        if( element.timers == null) {
            element.timers = new ArrayList<MGTimer>();
        }/* else if( element.timers.size() == element.timers )
            ccArrayDoubleCapacity(element->timers);
		*/
        MGTimer timer = new MGTimer(target, selector, interval);
    ...
    public void schedule(UpdateCallback callback, Object target, float interval, boolean paused) {
        ...
        if( element.timers == null) {
            ...
            ccArrayDoubleCapacity(element->timers);
		*/
        MGTimer timer = new MGTimer(target, callback, interval);
        element.timers.add(timer);
    }
    ...
    public void unschedule(String selector, Object target) {
        ...
        if( element != null ) {
            for( int i=0; i< element.timers.size(); i++ ) {
                MGTimer timer = element.timers.get(i);
...
//            	this.removeHashElement(element.target, element);
    ...
    */
	public void scheduleUpdate(Object target, int priority, boolean paused) {
        ...
        if (MGConfig.FRAMEWORK_DEBUG >= 1) {
            ...
            assert hashElement == null:"MGScheduler: You can't re-schedule an 'update' selector'. Unschedule it first";
        }
    ...
    */
    public void scheduleTimer(MGTimer timer) {
     ...
     */
    public void unscheduleTimer(MGTimer timer) {
